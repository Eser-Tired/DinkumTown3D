...
# —— 地图种子 ——
# 这一局的地形种子（起伏 / 河流走向 / 植被分布全由它派生）。
# main 在世界建好之后写进来，存档系统把它塞进 __meta；
# 读档时反过来：先从 __meta 取出来，再据此重建地形。
# 放这里而不是 main 里，是因为存档时 main 模块的数据要等 load() 之后才反序列化，
# 而地形必须在那之前就位——元数据是唯一能"提前读到"的位置。
var terrain_seed := 20260921
